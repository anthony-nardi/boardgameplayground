...
class GamePieceRenderer {
  ...
  drawGamePiece(gamePiece, canvas) {
    ...
    if (gamePiece.ownedBy === PLAYER_ONE) {
      ...
      context.beginPath();
      context.arc(this.circleRadius, this.circleRadius, this.circleRadius - 2, 0, 2 * Math.PI);
      context.fill();
    } else {
    ...
    if (gamePiece.type === TZAAR) {
      ...
      context.beginPath();
      this.drawStar(this.circleRadius, this.circleRadius, 4, this.circleRadius / 2, this.circleRadius / 4, context, this.CENTER_COLOR);
